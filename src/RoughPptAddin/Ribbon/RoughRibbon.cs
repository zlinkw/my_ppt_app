...
public class RoughRibbon : IRibbonExtensibility
{
	...
	{
		tab.AppendChild(output.ImportNode(FindRibbonElement(source, groupId), deep: true));
	}

	private static XmlElement FindRibbonElement(XmlDocument document, string id)
	{
		return (document.SelectSingleNode("//*[@id='" + id + "']") as XmlElement) ?? throw new InvalidOperationException("Ribbon control is missing: " + id);
	}
