...
- 打包后视觉复核（已修正，待重打包）：首次 `0.1.743` 三件套生成成功，但帮助截图暴露隐藏预览图仍被作者样式强制显示为破图占位，Plotly 长按钮文字被省略。现已恢复 `[hidden]` 语义、允许网站按钮文字自然换行并更新真实截图；重新运行完整 `npm.cmd test` 与 `npm.cmd run build:ui` 均通过。旧安装包不交付，等待基于修正提交重新打包。
