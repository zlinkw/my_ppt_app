...
2. R002（待处理）：恢复已提交但未打包的 B503 横向可读布局、拖动滚动及回归合同。
3. R003（待处理）：核对恢复源码、脚本、测试链与 `0.1.695` 删除前基线，缺失项按会话精确恢复。
4. R004（待处理）：配色只使用当前单张参考图的全部实际色系；未保存时切换图片需页面内确认，并支持仅本次 PowerPoint 会话免询问。
