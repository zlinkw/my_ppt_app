...
5. B571-B574（已完成）：补齐科研绘图筛选输入可访问名称；校验形状目录和本地图库偏好的异常数据，防止空白图库或渲染中断。
6. B575（已完成）：按维护规则压缩目标计划历史。
7. B576（已完成）：为科研绘图单选按钮组补齐方向键、Home 和 End 键盘导航，并建立真实浏览器回归。
8. B577（已完成）：修复图表筛选隐藏当前选择后单选组没有可见选中项的问题。
9. B578（已完成）：校验本机特征块默认值，防止损坏偏好把非法类型或范围传入插入链路。
10. B579（已完成）：校验本机风格模板参数，防止损坏偏好把 NaN 或非法枚举传入手绘生成。

### 下一批次方向

- 继续按 `PROJECT_CONSTRAINTS.md` 5.1 优先做 UI：外观、布局、发现性、首屏密度、图标一致性、状态可读性。
- 每批只做一个同风险面改动，新增可见控件必须同时补中文文案、tooltip 和对应静态合同。
- 下一个统一验证节点为 B576-B580；节点前不运行 `npm test`、UI 构建或打包。
...
- B571-B575 统一验证：完整 `npm.cmd test` 与 `npm.cmd run build:ui` 全绿；无产物漂移。按“未明确要求时不打包”边界未产出安装包。
- B576：图表类型和配色使用 `role=radiogroup` 时必须遵守单选键盘语义；方向键移动并选择可见项，Home/End 跳到首尾，隐藏项不参与导航。
- B576 验证与提交：真实浏览器验证图表类型右/左/上/Home 及配色下方向键移动、选择、焦点和 roving tabindex；`validate-research-chart-keyboard.mjs` 已加入 `npm test` 并通过。代码与本批计划同提交。
- B577：搜索或类别筛选导致当前图表不可见时，必须自动选择首个可见图表，保证单选组始终有可见 `aria-checked=true` 项。
- B577 验证与提交：真实浏览器搜索“雷达”后验证旧选择被隐藏、雷达成为唯一可见选中项且标题同步；科研绘图静态合同和键盘回归通过。代码与本批计划同提交。
- B578：特征块默认值必须按枚举、数值范围、整数和颜色格式重建，丢弃未知字段；布尔值只接受严格 `true` 或默认值。
- B578 验证与提交：`app.mjs` 语法检查通过；`validate-taskpane-resource-guards.mjs` 与 `validate-taskpane-action-wiring.mjs` 通过。代码与本批计划同提交。
- B579：本机风格模板参数必须按颜色、布尔、枚举和数值范围重建；无效值回退内置默认，未知字段不进入生成链路。
- B579 验证与提交：`app.mjs` 语法检查通过；`validate-taskpane-resource-guards.mjs`、`validate-style-panel-sync.mjs` 和 `validate-taskpane-action-wiring.mjs` 通过。代码与本批计划同提交。
