...
3. R003（五批统一验证节点）：R003-A 已补回 Zotero 完整图库入口，R003-B 已补静态合同，R003-C 已恢复删除前完整 `npm test` 链和 `sync-ui-output.mjs`；五个小批次已累计完成，下一步统一测试、UI 构建与打包。保护用户安装态，不执行含进程终止的 `package-run.ps1`。
