...
7. B576-B579（已完成）：补齐科研绘图单选键盘导航和筛选选中保护；校验特征块默认值与风格模板参数，阻断损坏本机偏好进入生成链路。
8. B580（已完成）：按维护规则压缩目标计划历史。
9. B581（已完成）：修复 B579 遗漏的三个风格参数规则，恢复任务窗格脚本初始化。
10. B582（已完成）：校验本机形状排序偏好，防止损坏值进入形状列表排序分支。
11. B583（已完成）：限制本机最近、收藏和命令偏好的初始数量，防止损坏的超长数组放大首屏渲染。

### 下一批次方向

- 继续按 `PROJECT_CONSTRAINTS.md` 5.1 优先做 UI：外观、布局、发现性、首屏密度、图标一致性、状态可读性。
- 每批只做一个同风险面改动，新增可见控件必须同时补中文文案、tooltip 和对应静态合同。
- 下一个统一验证节点为 B582-B586；节点前不运行 `npm test`、UI 构建或打包。
...
- B581 故障记录：B579 的参数规则缺少 `fillWeight`、`hachureGap` 和 `curveFitting`，`pickStyleParams` 在初始化内置模板时读取 `undefined.type`，导致整页模块加载失败。修复后必须重新执行 B576-B580 统一验证。
- B581 验证与提交：真实浏览器确认 `roughPptTaskPaneReady=true`；资源守卫、任务窗格交互和风格同步合同通过。代码与本批计划同提交。
- B582：形状排序偏好只接受 smart、favorites、recent、az；其它本机值回退 smart。
- B583：本机偏好读取后必须立即裁剪到运行上限：形状最近/收藏 12、命令 8、论文最近 12、收藏 16，独立图库同限 12。
- B583 验证与提交：两侧脚本语法检查通过；资源守卫、形状图库合同和独立窗口布局验证通过。代码与本批计划同提交。
- B582 验证与提交：`app.mjs` 语法检查通过；资源守卫和任务窗格交互合同通过。代码与本批计划同提交。
- B576-B580 复验：B581 纠偏后，完整 `npm.cmd test` 与 `npm.cmd run build:ui` 全绿；无产物漂移。按“未明确要求时不打包”边界未产出安装包。
