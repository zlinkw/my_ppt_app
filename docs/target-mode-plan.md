...
16. B600（已完成）：补齐更新消息的桥接合同；B596-B600 统一验证通过，下一个节点为 B601-B605。
17. B601（已完成）：风格模板、快捷样式和参数跳转横向列表支持鼠标拖动与键盘滚动；加宽模板卡提升名称和摘要可读性，收起态状态条隐藏滚动条但保留换行。
18. B602（已完成）：补齐更新服务的空响应、GitHub 频率限制和连接失败中文边界。
19. B603（已完成）：重拍总览和风格说明截图，使示例状态保持收起成功态，并反映加宽后的模板卡与横向拖动列表。
20. B604（已完成）：重拍特征块说明截图，使特征块示例同步“检查更新”入口、收起成功状态和当前参数布局。
...
- 下一个统一验证节点为 B601-B605；节点前不运行 `npm test`、UI 构建或打包。
...
- B600：前端新增宿主消息时必须同步 `bridge-contract.mjs`。B596-B600 统一验证中，完整 `npm.cmd test`、`npm.cmd run build:ui` 和 Release `scripts/build.ps1` 全部通过；Release 构建确认新 C# 服务可编译并完成 Ribbon 图标验证，产物仅暂存未打包安装。
- B601：横向内容必须提供拖动、方向键、中文 aria-label 和提示；预览卡不得用过窄固定宽度截断关键文本；状态条收起时隐藏滚动器，展开时仍保留长文滚动。
- B602：网络异常必须转成可操作中文提示；空响应和限流不得暴露原始空引用或让用户误判插件故障。
- B603：说明截图不得把浏览器无宿主的长错误展开态当成正常用户状态；示例应展示绿色收起状态、真实上下文徽标和当前控件尺寸。
- B604：所有任务窗格说明图必须使用同一顶栏结构和状态语义；上下文专属示例只显示该选区可用的工作区。
