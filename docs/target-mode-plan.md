...
9. B611（阻塞）：本地开发构建元数据代码已就位；完整构建在跨仓合同处停止，外部 Zotero 工作区把视图版本改为 38 而 PPT 冻结合同仍要求 37。不得修改外部仓或放宽合同掩盖漂移。
...
13. B615（待做）：复核已安装任务窗格、形状图库和科研绘图窗口的截图与布局探针；B611-B615 后统一验证。
