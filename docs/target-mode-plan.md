...
5. B529（进行中）：补齐数据筛选、配置持久化、SVG 下载与使用说明；完成五批次统一测试、构建和打包。
6. B530（待处理）：直接移除完整模式中重复的 SimpleExperiment 与 Zotero 连接状态按钮，保留上方现有功能入口。
...
- B529 状态：数据筛选、配置持久化、当前预览 SVG 下载与使用说明已实现；完整 `npm.cmd test`、UI 构建、79 个 Ribbon 图标验证和签名 Release 构建通过，待本批提交后生成三种安装产物。
