...
6. B659（已完成）：Office 16.0.20326 实测确认裁剪后直接转换会生成隐藏区图形；新增“裁剪后转图形”，按 PowerPoint 记录的图像偏移校正位置，再对边界原生图形取矩形交集。左右上下及组合裁剪、组合图形和未裁剪原图保留验证通过；科研绘图合同、JS 语法及 Release 编译通过。复杂描边和渐变可能受 Office 转换影响，裁剪边缘需人工核对。普通 SVG 插入和原有可编辑插入保持不变。
...
10. B663（已完成）：Tavotto v0.15.0 固定随 Windows 安装包交付；发布锁定清单校验上游 SHA256，载荷含桌面程序、官方 CLI、AGPL 许可证和完整源码归档，插件优先调用内置 CLI，保留外部覆盖入口。真实 CLI `doctor/open --no-launch`、`npm run package` 内含的完整测试、UI 与 Release 构建、ZIP 载荷及三种安装包哈希核对通过。首轮 IExpress 因 90 秒等待上限误报超时，延长到 900 秒后重打包成功；版本 `0.1.931`，位置 `releases/RoughPptAddin-0.1.931-1fd6e18b`。代码提交 `83ed1d4`、`f9b010d`、`1fd6e18` 已同步 `origin/master`。未安装插件或启动桌面编辑器，真实交互尚待使用时核对。固定版本不会随上游发布自动变更。
11. B664（已完成）：0.1.931 真实桌面验证发现 Tavotto v0.15.0 不扫描独立 SVG、仅导出 PDF/PNG；固定 PyMuPDF 1.28.2 及源码随包交付，SVG 先转矢量 PDF 交接，导出的单页 PDF 转 SVG 后沿原安全校验链路导回。桌面端实际添加素材、修改尺寸与位置、导出 PDF；插件服务真实交接与 PDF 导回、PPT SVG 插入通过。`npm run package` 内含完整测试、UI 与 Release 构建通过；0.1.936 三格式安装包与清单哈希通过，MSI/EXE 由 `CN=RoughPptAddin Dev` 签名并加时间戳，清单带 `.p7s`，ZIP 哈希被清单覆盖。已用签名 MSI 覆盖安装，退出码 0，已安装版本 `0.1.936 / 1ca07246`、`dirty=false`、`LoadBehavior=3`，PowerPoint 中加载成功。代码提交 `47fce70`、`6b3fdd1`、`1ca0724` 已同步 `origin/master`。自签开发证书只在显式信任的设备有效，公开分发仍需组织签发的代码签名证书；含嵌入位图的 PDF 可能被 SVG 安全校验拒绝。未改动其他科研绘图入口。
