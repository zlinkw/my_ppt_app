...
14. B646（已完成）：Ribbon 图库与任务窗格形状图标轨道改为固定密度网格，避免少量形状类别把每个图标拉伸成超宽格子。
15. B647（已完成）：移除未被引用的旧形状卡片渲染函数，避免后续维护误用含嵌套交互的旧实现。
16. B648（已完成）：形状卡固定按钮改为悬停或键盘聚焦时显示，静置图库不再被 203 个固定徽标占据视觉注意力。
17. B649（已完成）：B645-B648 统一验证后生成 0.1.906 三格式安装包，MSI 无感安装并核对安装载荷。
18. B650（已完成）：在线说明和使用指南补充悬停或键盘聚焦后固定形状的操作方式，避免隐藏按钮降低发现性。
19. B651（已完成）：修复安装前置构建失败后仍继续注册旧本体的问题，并为任务窗格交互验证增加前端初始化完成门槛；同时把无障碍布局验证纳入全量测试。
20. B652（已完成）：统一放大 Ribbon 图库和右侧形状预览的绘制画布、显示尺寸与网格密度，减少小图形在空白单元中的清晰度损失。
21. B653（已完成）：移除完整模式空搜索时的重复命令中心，只保留搜索命中结果；命中结果改为单列可读卡，避免右侧再展示 Ribbon 高频命令。
22. B654（已完成）：把快捷工作台的模式切换按钮改为双向动态切换，完整模式显示“简洁模式”，简洁模式显示“完整模式”，并同步中文悬浮说明。
23. B655（已完成）：宽窗简洁模式内容列限制到 640px 并水平居中，避免卡片和空白区在宽任务窗格两侧过度拉伸。
24. B656（已完成）：B651-B655 统一验证后生成 0.1.913 三格式安装包，MSI 无感安装并核对安装载荷与已安装前端。
25. B657（已完成）：按当前图标密度、命令结果和简洁模式布局重建帮助截图，避免在线说明展示过期界面。
...
- B646 验证锚点：Ribbon 图库、任务窗格图库、窗口布局和 UI 合同通过；700px 图库截图确认最近、线条和基础形状使用一致的 42 至 48px 图标轨道，截图已回收。
- B647 验证锚点：`node --check`、源码约束、任务窗格图库和 UI 合同通过；`renderCard` 无引用且已从前端源码移除。
- B648 验证锚点：任务窗格图库、任务窗格交互和 UI 合同通过；800px 静置截图确认形状预览无固定按钮遮挡，键盘聚焦卡片后按钮仍可发现。
- B649 验证锚点：`npm run package` 全绿；`releases\RoughPptAddin-0.1.906-8d643ff7` 生成 ZIP、MSI 和 EXE。MSI 静默返回 0，安装元数据为 `0.1.906 / 8d643ff77004`、`dirty=false`、`LoadBehavior=3`，DLL、清单和 UI 关键哈希一致。
- B650 验证锚点：编码、UI 合同和非模态帮助合同通过；USER_GUIDE 与 help.html 均说明悬停或键盘聚焦后使用右上角固定按钮。
