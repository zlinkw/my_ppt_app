...
9. B588（已完成）：完成四页 320–1400px 响应式、对比度和键盘焦点审计，未发现新缺陷。
10. B589（已完成）：为图文说明增加跳到正文链接，改善键盘用户的长目录导航。
...
- B588：320–1400px 响应式审计、四页对比度审计和 300 多个控件焦点审计均为零缺陷。
- B589：帮助页必须提供跳到正文链接；目标正文可聚焦；真实浏览器点击后哈希和焦点都必须落到 `#guideMain`。
- B589 验证与提交：说明窗口静态合同和真实浏览器跳转焦点回归通过；本地资源合同通过。代码与本批计划同提交。
