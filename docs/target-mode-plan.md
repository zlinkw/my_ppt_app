...
4. B564（已完成）：用当前真实界面重新生成四张使用说明截图，消除旧版布局和入口造成的图文不一致。
5. B565（已完成）：修复科研绘图同源 SVG 预览在超高容器中的内部垂直留白，并让布局合同检查实际可见的导入预览而非隐藏的生成节点。
6. B566（待执行）：继续 UI 审计，优先处理剩余的可验证外观、尺寸或功能反馈缺口。
...
- B564：使用说明 PNG 是交付资产，必须随界面结构变化重新生成；截图内容需与对应章节的入口和工作区状态一致。
- B565：同源 SVG 以 `<img>` 展示时不得用固定 100% 高度加 `object-fit: contain` 在超高容器中制造上下留白；布局验证必须选择当前可见预览节点。

### B561-B565 统一验证

- 完整 `npm.cmd test` 与 `npm.cmd run build:ui` 全绿；无产物漂移。按“未明确要求时不打包”边界未产出安装包。
