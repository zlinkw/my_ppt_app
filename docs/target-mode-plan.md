...
7. B573（已完成）：校验本机最近形状和收藏形状必须是字符串数组，防止损坏偏好中断图库渲染。
8. B574（已完成）：为独立形状图库应用同一偏好校验合同，避免两侧行为分叉。
...
- B573：本机形状偏好必须过滤为字符串数组；损坏 JSON 或错误类型只能回退默认，不得中断渲染。
- B574：独立图库和右侧窗格的数组偏好必须使用相同的类型校验与回退规则。
