...
1. B524：移除简洁模式重复的科研绘图与论文图像按钮；此前已完成批次由 Git 历史追溯。

### B524 简洁模式重复入口清理

- 状态：已完成。
- 目标：从 DOM、运行时接线和样式中直接移除简洁模式内重复的 `SimpleExperiment 科研绘图` 与 `论文图像` 按钮，仅保留上方现有入口和“完整模式”切换按钮。
- 范围：任务窗格 HTML、交互接线、重复按钮专属 CSS、相邻 UI 合同。
- 排除：不移除上方快捷入口、完整模式连接状态条或“完整模式”按钮；不改变科研绘图、ZLK、Zotero 功能行为。
- 保护区：不打包、不安装、不关闭或重启 PowerPoint/VS Code；本批不运行统一测试或 UI 构建。
- 回归检查：两个重复按钮及其专属选择器、状态更新和拖动接线均不存在；“完整模式”按钮仍可切换；上方入口与完整模式连接状态条保留。
- 验证：`node --check`（`app.mjs` 与两个验证脚本）、`git diff --check`、源码静态残留断言通过；按五批次协议未运行统一测试、构建或打包。
- 未决风险：待后续五批次统一 UI 构建与完整测试确认实际任务窗格布局；下一边界为后续独立 UI 问题批次。
- 提交记录：`8fcd734 ui: remove duplicate simple mode buttons`。
...
- B523（统一验证与发布完成）：使用说明、架构、合同和真实截图已同步；修正首次扫描失败后，完整测试、UI 构建、签名 Release 和 79 个 Ribbon 图标均通过。`releases/RoughPptAddin-0.1.750-482489a0/` 的 ZIP、MSI、EXE 与 manifest/SHA256 一致，内置 `482489a0a0d1`、`master`、`dirty=false`；未自动安装，待用户手动验证。
