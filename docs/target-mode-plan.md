...
11. B664（进行中）：用户要求本机安装、验证 Tavotto 桌面编辑并签发安装包。0.1.931 安装与 CLI 启动通过；真实桌面验证发现 Tavotto v0.15.0 不扫描独立 SVG、仅导出 PDF/PNG，原“当前 SVG 交接/导回 SVG”入口不能完成往返。范围为固定的矢量 SVG↔PDF 转换载荷、交接和导回接线、桌面编辑/导出回归、安装器签名与清单核对；保留既有入口，使用本机开发证书时必须标明非公开可信签名。每批不超过 8 文件，相关验证后同步 `origin/master`。本次用户明确授权安装和桌面验证，覆盖通常不自动安装的项目约束。
