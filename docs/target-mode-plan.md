...
3. R003（部分恢复，待五批统一验证）：R003-A 已补回 Ribbon“打开论文图片库”唯一入口；严格从系统临时目录打开 Zotero 生成的 `paper-image-library.html`，bridge token、ready 状态和固定 endpoint 有效时先发 `refreshLibrary`，离线仅只读打开已有页面，无页面则中文提示启动 Zotero；未复制或生成第二套图库。R003-B 已补充静态合同，覆盖唯一页面、命令白名单、单图配色与会话确认；该小批次仅完成差异检查，未运行测试、构建或打包。
...
5. R005（待处理）：更新使用说明；累计五个小批次后统一运行完整测试、UI 构建与打包，不自动安装。
