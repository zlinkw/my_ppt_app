...
5. B567（已完成）：为使用说明目录增加滚动高亮，让用户阅读长文档时始终知道当前章节。
6. B568（已完成）：补齐形状目录读取失败的中文反馈，消除空白图库和静默降级。
7. B569（已完成）：让右侧形状目录降级提示不被后续初始化状态覆盖。
8. B570（已完成）：合并初始化尾部的宿主缺失与目录降级提示，避免一条重要状态覆盖另一条。
9. B571（待执行）：继续按计划审计剩余 UI 与功能改进点。

### 下一批次方向

- 继续按 `PROJECT_CONSTRAINTS.md` 5.1 优先做 UI：外观、布局、发现性、首屏密度、图标一致性、状态可读性。
- 每批只做一个同风险面改动，新增可见控件必须同时补中文文案、tooltip 和对应静态合同。
- 下一个统一验证节点为 B566-B570；节点前不运行 `npm test`、UI 构建或打包。
...
- B566：真实浏览器验证 320/420px 顶栏四行流式排布，状态占满可用行且版本按钮在下方。
- B566-B570 统一验证：完整 `npm.cmd test` 与 `npm.cmd run build:ui` 全绿；无产物漂移。按“未明确要求时不打包”边界未产出安装包。
- B567：使用说明目录高亮必须由滚动位置驱动，使用 rAF 合帧，初始和跳转后都要准确；所有十个链接保留中文说明。
- B567 验证与提交：真实浏览器验证初始命中快速开始，点击科研绘图后仅该目录项 `aria-current=true`；`validate-usage-guide-modeless.mjs` 已纳入该运行时回归并通过。代码与本批计划同提交。
- B568：目录缺失或响应无效时，独立图库必须显示中文错误；右侧窗格若使用四形状兜底，也必须在状态条说明当前不是完整目录。
- B568 验证与提交：`app.mjs` 与 `ribbon-shape-gallery.mjs` 语法检查通过；合同锁定右侧兜底提示、独立图库状态和空目录占位；`validate-taskpane-shape-gallery.mjs` 通过。代码与本批计划同提交。
- B569：目录降级是持续会话风险，状态提示必须保留到用户看到；不能只短暂闪现后被宿主连接或资源刷新文案覆盖。
- B569 验证与提交：目录降级标记保留到初始化尾部并重新显示中文错误；`validate-taskpane-shape-gallery.mjs` 与 `validate-ui-contract.mjs` 通过。代码与本批计划同提交。
- B570：宿主连接缺失和目录降级可以同时存在；初始化尾部必须合并显示，不允许后一条覆盖前一条。
- B570 验证与提交：脚本语法通过；合同锁定 `startupIssues` 合并宿主和目录状态；`validate-taskpane-shape-gallery.mjs` 与 `validate-ui-contract.mjs` 通过。代码与本批计划同提交。
