...
The package includes Tavotto v0.15.0 as a separate desktop program under `publish\third-party\Tavotto`. Packaging downloads only the pinned upstream release and source commit recorded in `scripts\tavotto-bundle.lock.json`, verifies SHA256, extracts the Windows desktop payload, and carries its AGPL license and full upstream repository source archive. The add-in invokes that bundled CLI before looking for a separately installed Tavotto. The bundled version stays fixed until the lock file is deliberately changed; no Tavotto update is fetched during installation.
