...
`scripts\package-installers.ps1` wraps that package into versioned Windows installers under `releases\RoughPptAddin-{installerProductVersion}-{shortCommit}\`: `RoughPptAddin-Windows11.zip`, `RoughPptAddin-Windows11.msi`, and `RoughPptAddin-Windows11-Setup.exe`, plus `installer-manifest.json` and `installer-build\` work files (WiX `.wxs`/`.wixobj`, IExpress `.sed`). The MSI and EXE extract the same native editable VSTO add-in payload and run `scripts\install.ps1 -SkipBuild -InstallPrereqs`. Pass `-ReleaseRoot` to override the versioned directory; without it an existing directory gets a `-rN` suffix and never overwrites. `dist\` remains only a staging area (`dist\RoughPptAddin`, `dist\RoughPptAddin.zip`); final shippable artifacts live only under `releases\` and `scripts\verify-deploy-package.ps1` resolves them via `-ReleaseRoot` or the newest `releases\RoughPptAddin-*` directory.
...
The package includes Tavotto v0.15.0 as a separate desktop program under `publish\third-party\Tavotto`. Packaging downloads only the pinned upstream release and source commit recorded in `scripts\tavotto-bundle.lock.json`, verifies SHA256, extracts the Windows desktop payload, and carries its AGPL license and full upstream repository source archive. A pinned PyMuPDF 1.28.2 wheel and source archive provide local vector SVG to PDF handoff and PDF to SVG return, since this Tavotto release scans PDF/PNG assets and exports PDF/PNG, not standalone SVG. The add-in invokes the bundled CLI before looking for a separately installed Tavotto. The bundled version stays fixed until the lock file is deliberately changed; no Tavotto update is fetched during installation.
Packaging derives the MSI `ProductVersion` from the persisted `installerVersionBaseline` plus the current git commit count, then records it as `installerProductVersion` in the installer manifest under the same versioned `releases\` directory. The baseline preserves monotonic upgrades after repository recovery, so packages remain newer than already installed historical builds even when the recovered git history is shorter. Packaging also writes SHA256 hashes for the portable zip, MSI, and EXE into that release-local `installer-manifest.json`.
...
`npm run package` signs the MSI and EXE with that CurrentUser development certificate and a SHA-256 timestamp, then signs `installer-manifest.json` as a detached `.p7s` file. The signed manifest authenticates the ZIP hash. Run `pwsh scripts/verify-release-signature.ps1 -ReleaseRoot <release directory>` on the signing machine to verify all three artifacts. This self-signed certificate is trusted only where explicitly installed; public distribution requires an organization-issued code-signing certificate.
