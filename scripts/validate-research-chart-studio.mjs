...
if ((html.match(/data-chart-type=/g) || []).length !== 32) throw new Error("科研绘图工作区必须保留 32 种图表入口。");
...
for (const chartType of ["bar", "groupedBar", "stackedBar", "horizontalBar", "line", "step", "area", "scatter", "bubble", "histogram", "boxplot", "density", "violin", "ecdf", "strip", "regression", "qqPlot", "ppPlot", "forest", "roc", "precisionRecall", "calibration", "blandAltman", "volcano", "funnel", "survival", "cumulativeHazard", "heatmap", "correlationMatrix", "parallelCoordinates", "donut", "polar"]) {
...
for (const snippet of ['state.chartType === "qqPlot"', 'state.chartType === "ppPlot"', "buildProbabilityPlotRows", "normalCdf", "normalQuantile", "__theoreticalQuantile", "__empiricalProbability", "__expectedQuantile", "至少需要三个有效数值"]) {
  if (!app.includes(snippet)) throw new Error(`科研绘图分布诊断合同缺少：${snippet}`);
}
...
for (const snippet of ["Vega Lite", "32 种图表", "相关矩阵", "并行坐标", "Q-Q", "P-P", "小提琴", "经验累积分布", "森林图", "ROC", "精确率召回", "校准曲线", "Bland", "火山图", "漏斗图", "Kaplan", "累计风险", "数据筛选", "保存配置", "下载 SVG", "实时预览", "同一份 SVG", "PowerPoint 2016", "插入 PPT", "不会自动打开浏览器"]) {
